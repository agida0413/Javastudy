/*
 * 데이터 저장 =====> 데이터 가공(데이터 처리) ====>가공된 데이터 출력
 * 										 사용자 요청
 * 					사용자 요청 처리==>로그이니 ,검색결과
 * 						 |
 * 					  1.연산자
 ...
 * 		       생성자, 변수 , 메소드
 * 		             ---  ----
 * 	1.사용 목적 =>1.가독성 ,2.재사용성, 3.속도(최적화) 4.
 * 	  = 분석 (가독성) =  >수정 (유지보수), 오류처리
 * 	    기능별 분리 (로그인 처리,회원가입,목록 ,페이지)
 *    = 다른 클래스와 연결(메시지)
 ...
 *        =>데이터 읽기 : getter 
 *        
 *   3.메소드 제작시 고민  == > 리턴형은 한개만 사용할수 있다 .(결과값이 하나다.)(여러개면 클래스나 배열로 묶어서 리턴)
 *   	--------------------------------------------
...
public class 메소드_1 {
	static void a() {
		...
		System.out.println("a()사용자 요청 처리종료");
